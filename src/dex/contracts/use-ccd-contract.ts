...
import contractNames from "@dex/config/contractNames";
import { bridgeManager, cis2Bridgeable } from "@dex/config/schemas";
import leb from "leb128";
import { Buffer } from "buffer/index";
import { Components } from "@dex/api-query/__generated__/AxiosClient";
import decodeOperatorOf from "@dex/helpers/decodeOperatorOf";
import { detectConcordiumProvider } from "@concordium/browser-wallet-api-helpers";
import { collapseRatio } from "@dex/helpers/ccd-node";
...
const useCCDContract = (ccdAccount: string | undefined, enabled: boolean) => {
    const bridgeManagerContract = {
        index: addresses.bridgeManager.index !== undefined ? BigInt(addresses.bridgeManager.index) : undefined,
        subindex: addresses.bridgeManager.subindex !== undefined ? BigInt(addresses.bridgeManager.subindex) : undefined,
    } as ContractAddress;
    ...
    const approve = async (token: Components.Schemas.TokenMapItem, maxContractExecutionEnergy: bigint) => {
        ...
        const contractAddress = {
            index: token.ccd_contract?.index !== undefined ? BigInt(token.ccd_contract?.index) : undefined,
            subindex: token.ccd_contract?.subindex !== undefined ? BigInt(token.ccd_contract?.subindex) : undefined,
        } as ContractAddress;
    ...
    const hasApprove = async (ccdTokenAddress: { index: number; subindex: number }) => {
        ...
        // calculateEnergyCost
        // https://github.dev/Concordium/concordium-browser-wallet/blob/main/packages/browser-wallet/@dex/popup/pages/SendTransaction/SendTransaction.tsx#L83
