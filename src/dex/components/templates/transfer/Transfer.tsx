...
import { TokenWithIcon, useTokens } from "@dex/api-query/queries";
...
import parseWallet from "@dex/helpers/parseWallet";
    ...
    return (
        <MaxGapRow>
            ...
            <Button variant="max" onClick={() => onMax(maxTransferValue ?? selectedTokenBalance ?? "")}>
                <Text fontColor="#0095c8" fontSize="18" fontWeight="bold">
                    Max
                </Text>
            </Button>
...
const Transfer: React.FC<Props> = ({ isDeposit = false, maxTransferValue }) => {
    ...
    const { balanceOf: getCcdTokenBalance } = useCCDContract(ccdContext.account, ccdContext.isActive as boolean);
        ...
        <PageWrapper>
            ...
            >
                <LinkWrapper hidden={!context.account}>
                    <Text fontSize="18" fontWeight="bold" fontFamily="Roboto" fontColor="White">
                        TRANSACTION HISTORY
                    </Text>
                </LinkWrapper>
            </Link>
